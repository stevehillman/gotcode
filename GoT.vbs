...
Const VolDef = 0.9		' Default volume for callouts and sound effects
...
Sub Table1_KeyDown(ByVal Keycode)
    ...
    If bGameInPlay AND NOT Tilted Then
        ...
        If keycode = LeftFlipperKey Then 
            If Not WHCFlipperFrozen Then SolLFlipper 1
...
Sub Table1_KeyUp(ByVal keycode)
    ...
    If bGameInPLay AND NOT Tilted Then
        If keycode = LeftFlipperKey Then
            If Not WHCFlipperFrozen Then SolLFlipper 0
...
Sub Drain_Hit()
    ...
    If(bGameInPLay = True)AND(Tilted = False)Then
        ...
        Else
            ...
            If(BallsOnPlayfield-RealBallsInLock = 0 And Not tmrBWmultiballRelease.Enabled)Then
                ' End Mode and timers
...
Class cPState
    ...
    Public Sub Restore
        ...
        bHotkReady = myHotkMBReady
        bHotkDone = myHotkMBDone
...
Class cHouse
    ...
    Private Sub Class_Initialize(  )
        ...
        QualifyValue = 100000
        bBattleReady = False
        LockWall.collidable = False : Lockwall.Uservalue = 0 : LockPost.TransZ = 0 : Locksword.ObjRotY = -30
        UPFState = 0
        UPFLevel = 1
        UPFMultiplier = 1
    ...
    Public Property Let MyHouse(h) 
        ...
        If h = Baratheon Then AdvanceWallMultiball 5 : WallJPValue = 9250000 Else WallJPValue = 3400000
    ...
    Public Sub ResetLights
        ...
        Next
        CompletedHouses = j
        if bBattleReady And Not bMultiBallMode Then SetLightColor li108,white,2 Else SetLightColor li108,white,0
        If bHotkMBDone Then SetLightColor li29,white,1
        'TODO: Set IronThrone lights too
    End Sub

    Public Sub SetShieldLights
        Dim i,j,clr
        If bBWMultiballActive Then clr = green 
        Elseif bCastleMBActive Then clr = cyan
    ...
    Public Sub SetJackpots
        ...
        ElseIf bHotkMBActive Then
            For i = 1 to 7
                If HotkState = 2 Then BWJackpotShots(i) = 99
                ElseIf HotkState = 0 And ((HotkMask And 2^i) > 0 Or (HotkMask And 2^Baratheon) > 0 ) Then BWJackpotShots(i) = 1
                End If
            Next
        Else
            For i = 1 to 7
        ...
        if PlayerMode = 0 And bMultiBallMode = False Then
            ...
            Else
                ...
                ElseIf h = Targaryen Then
                    PlaySoundVol "gotfx-dragonroar1",VolDef
                End If
            End If
    ...
    Sub HouseCompleted(h)
        ...
        If bCompleted(h) = False Then
            ...
            If CompletedHouses = 4 Then 
                ...
                Next
            End If
            If CompletedHouses >=4 Then CheckForHotkReady
            If h = Lannister Then
    ...
    Sub CheckBattleReady
        ...
        if Not bTargaryenInProgress Then
            ...
            Next
        End If
        If br or br1 And PlayerMode = 0 And Not bMultiBallMode Then Me.BattleReady = True Else Me.BattleReady = False
    End Sub
...
Class cBattleState
    ...
    Public Sub StartBattleMode
        ...
        Select Case MyHouse
            ...
            Case Martell: HouseValue = 0: tmr = 300 : State = 1 : GreyjoyMask = 10 : CompletedShots = 0 : OpenTopGates
            Case Tyrell
...
Sub VPObjects_Init
    ...
    vpmTimer.AddTimer 1000,"PlaySoundVol ""say-whathouse"",VolDef '"
End Sub
...
Sub ResetForNewGame()
    ...
    Wall075.collidable=True ' For debugging
    bGameInPLay = True
    GameTimeStamp = 0
...
Sub ResetForNewPlayerBall()
    ...
    if (House(CurrentPlayer).MyHouse = 0) Then
        ...
        If CurrentPlayer = 1 Then
            PlaySoundVol "say-choose-your-house2",VolDef
        Else
            PlaySoundVol "player"&CurrentPlayer,VolDef
            vpmTimer.AddTimer 1500,"PlaySoundVol ""say-choose-your-house2"",VolDef '"
        End If
        ChooseHouse 0
    Else 
...
Sub EndOfBall2()
    ...
    If(ExtraBallsAwards(CurrentPlayer) <> 0) and bBallInPlungerLane=False Then	' Save Extra ball for later if there is a ball in the plunger lane
        ...
        If bUseFlexDMD Then
            ...
            DMDEnqueueScene Scene,0,4000,8000,2000,"gotfx-dragonroar1"
...
Sub EndOfBallComplete()
    ...
    If((BallsRemaining(CurrentPlayer) <= 0)AND(BallsRemaining(NextPlayer) <= 0))Then
        ...
        SwordWall.collidable = False : LockSword.ObjRotY = -30
        LockWall.collidable = False : Lockwall.Uservalue = 0 : LockPost.TransZ = 0
    ...
    Else
        ...
        If PlayersPlayingGame > 1 Then
            i = RndNbr(2)
            PlaySoundVol "say-player" &CurrentPlayer&"-youre-up"&i, VolDef
...
Sub li89_Timer
    Dim i,bi
    For i = 1 to 5
        If ComboMultiplier(i) > 1 And (i <> 5 Or Not bWallMBReady) Then 
...
Sub sw38_Hit
    If Tilted then Exit Sub
    If bHotkMBReady Then PlaySoundVol "gotfx-ramp-hotk-ready",VolDef
...
Sub KickerFloor_Hit
    ...
    If ((((bMysteryLit And PlayerMode = 0) Or bWallMBReady And PlayerMode < 2) And Not bMultiBallMode) or bEBisLit) And Not bJustPlunged Then
        vpmTimer.AddTimer delay,"ElevatorKick 2 '"
        If Not bMultiBallMode Then FreezeAllGameTimers
    Else
        If Not bElevatorShotUsed Then bElevatorShotUsed = True : bCastleShotAvailable = True
...
Sub KickerIT_Hit
    ...
    If bEBisLit Then
        ...
        delay = 6500
    End If
    If bWallMBReady And PlayerMode < 2 And Not bMultiBallMode Then
        vpmTimer.AddTimer delay,"StartWallMB '"
    ElseIf bMysteryLit And PlayerMode = 0 And Not bMultiBallMode Then
...
Function MysteryAwardGoNoGo(aw)
    ...
    Select Case aw
        Case 2
            For i = 1 to 7
                If Not House(CurrentPlayer).Qualified(i) Then Exit Function
            Next
        ...
        Case 20: If Not bLoLUsed Then MysteryAwardGoNoGo = False
...
Sub SelectMysteryAward
    ...
    combo = ROrbitsw31.UserValue : If combo = 0 then combo = 1
    Select Case MysteryVals(MysterySelected)
        ...
        Case 26: DoEBisLit
        Case 28
   ' TODO: Include combo multiplier from ROrbitSW31.uservalue in Big Points
...
Sub StartWHCMB(shot)
    ...
    WHCMBScore = 0
    Dim balls : balls = 3
    If shot = Lannister Then balls = 4
    ...
    End If
    vpmTimer.AddTimer 1500, "PlaySoundVol ""say-whcstart"&RndNbr(2)&""",VolDef '"
    SetPlayfieldLights
    House(CurrentPlayer).SetUPFState true
...
Sub CheckForHotkReady
    Dim a
    If CompletedHouses < 4 Or bMultiBallMode Or PlayerMode > 0 Or bHotkDone Then Exit Sub
    bHotkMBReady = True
...
Sub StartHotkMBIntro
    ...
    ' Then switches to an offset 4 line display. "HAND OF THE KING" in top right, then house name 4 lines down on the left
...
' Do an UpdateHotkScoreScene sub
' create timers for sjp mode and stark bonus mode
' set up hurryup at the battering ram
' modify battering ram code to score the sjp
...
Sub SwordWall_Timer
    ...
    If RealBallsInLock > 0 or ((House(CurrentPlayer).bBattleReady Or bLockIsLit Or bHotkMBReady) And Not bMultiBallMode) Then 
        LockWall.Collidable = True : Lockwall.Uservalue = 1
        LockPost.TransZ = -60
    End If
...
Sub SetLockLight
    If bLockIsLit And Not bMultiBallMode Then
        ' Flash the lock light
        li111.BlinkInterval = 300
        SetLightColor li111,darkgreen,2
    Else
...
Sub InstantInfo
    ...
    Select Case InfoPage
        ...
        Case 11
            ...
            format=8:font="udmd-f6by8.fnt"
        Case 12
            If CompletedHouses >= 3 And Not bEBisLit Then InfoPage=13:InstantInfo:Exit Sub
        ...
        If IsEmpty(MysteryScene) Then
            ...
            For i = 0 to 2
                ...
                Select Case MysteryVals(i)
                    ...
                    Case 1,18,28
                        Dim combo : combo = ROrbitsw31.UserValue : If combo = 0 then combo = 1
...
Sub DMDCreateHotkMBScoreScene
    ...
    With scene.GetLabel("bonus")
        .SetAlignedPosition 54,16,FlexDMD_Align_Center
        .Visible = 0
    End With
    ...
    ' Only used for Stark bonus round
    scene.AddActor FlexDMD.NewLabel("tmr1",font,"20")
    With scene.GetLabel("tmr1")
        .SetAlignedPosition 127,13,FlexDMD_Align_TopRight
        .Visible = 0
    End With

    scene.AddActor FlexDMD.NewLabel("obj",font,"COMPLETE ALL SHOTS")
    scene.GetLabel("obj").SetAlignedPosition 64,9,FlexDMD_Align_Top
    BlinkActor scene.GetLabel("obj"),0.1,9999
    ...
    scene.GetLabel("sets").SetAlignedPosition 127,19,FlexDMD_Align_TopRight
...
Sub DMDLocalScore
    ...
    If bUseFlexDMD Then
        If IsEmpty(ScoreScene) And Not bAlternateScoreScene Then
            ...
            Set ComboFont = FlexDMD.NewFont("FlexDMD.Resources.udmd-f4by5.fnt", vbWhite, vbWhite, 0)
            If Score(CurrentPlayer) < 1000000000 Then font = "FlexDMD.Resources.udmd-f7by13.fnt" Else font = "udmd-f6by8"
...
' √? No houses were unqualified, but BattleReady didn't re-light after a battle

' √? WIC mode needs to turn off UPF lights
' √? WHC needs to turn off UPF lights
' √? should need to only hit one target to "complete" in WHC
' √? WHC only adds 2 balls
' √? BattleReady was lit during Castle MB
' - In Greyjoy vs Targaryen, Level 3 ended after hitting the dragon target, rather than going back to the previous state
' - In Greyjoy vs Baratheon, it wouldn't move to the next shot after hitting the dragon shot. Should have lit the Lannister shot too
